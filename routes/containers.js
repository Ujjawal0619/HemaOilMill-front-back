...
// @desc    Get all user containers data
// @access  Private
router.get('/', auth, async (req, res) => {
  try {
    ...
    const containersRecord = await Record.find({
      user: req.user.id,
      type: 'containers',
    }).sort({
      date: -1,
    });
    res.json(containersRecord);
  } catch (err) {
...
// @desc    Add new containers record
// @access  Private
router.post(
  '/',
  [
    ...
    [
      ...
      check('container', 'ContainerType is required').not().isEmpty(),
...
router.delete('/:id', auth, async (req, res) => {
  try {
    ...
    res.json({ msg: 'Record removed', type: 'containers' });
  } catch (err) {
    console.error(err.message);
    res.status(500).send('Server Error');
  }
