...
// @route   GET api/mustard
// @desc    Get all user mustard data
// @access  Private
router.get('/', auth, async (req, res) => {
  try {
    ...
    const mustardRecord = await Record.find({
      user: req.user.id,
      type: 'mustard',
    }).sort({
      date: -1,
    });
    res.json(mustardRecord);
  } catch (err) {
...
// @route   POST api/mustard
// @desc    Add new mustard record
// @access  Private
router.post(
  '/',
  [
...
router.delete('/:id', auth, async (req, res) => {
  try {
    ...
    res.json({ msg: 'Record removed', type: 'mustard' });
  } catch (err) {
    console.error(err.message);
    res.status(500).send('Server Error');
  }
